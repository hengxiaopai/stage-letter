...
from platform_adapters.douyin.adapter import DouyinAdapter  # noqa: E402
from platform_adapters.common import LiveStatus, ALL_STATES  # noqa: E402

ANCHORS_FILE = ROOT / "experiments" / "test_anchors" / "douyin.txt"
...
def load_anchors(path: Path) -> list:
    ...
    lines = []
    has_placeholder_warning = False
    for raw in path.read_text(encoding="utf-8").splitlines():
        line = raw.strip()
        if not line or line.startswith("#"):
            continue
        # 检测全大写英文占位符(用户没替换的 placeholder)
        if line.isupper() and "PLACEHOLDER" in line:
            has_placeholder_warning = True
            continue
        lines.append(line)
    if has_placeholder_warning:
        print(f"WARNING: {path} 包含未替换的 PLACEHOLDER,会跳过这些行", file=sys.stderr)
    return lines
...
def setup_logging(log_path: Path) -> logging.Logger:
    ...
    logger = logging.getLogger("douyin_24h")
...
def run(duration_seconds: int, interval_seconds: int, anchors: list, soak_type: str = "correctness") -> dict:
    ...
    log_path = DATA_DIR / f"douyin_24h-{ts}.log"
    jsonl_path = DATA_DIR / f"douyin_24h-{ts}.jsonl"

    logger = setup_logging(log_path)
    adapter = DouyinAdapter(min_interval=max(3, interval_seconds / 2))

    logger.info("=== Douyin 24h 稳定性实验启动 ===")
    ...
    try:
        ...
        while time.time() < end_at:
            ...
            for url in anchors:
                ...
                if result.get("ok"):
                    round_msgs.append(
                        f"[{result.get('room_id') or '?'}] state={state} ({latency_ms}ms)"
                    )
                else:
    ...
    summary_path = DATA_DIR / f"douyin_24h-{ts}.summary.json"
...
def main():
    p = argparse.ArgumentParser(description="Douyin 24h 稳定性实验")
    p.add_argument("--duration-hours", type=float, default=24.0)
    p.add_argument("--interval-seconds", type=int, default=600, help="抖音默认 10min(比 B 站长)")
    ...
    if not anchors:
        print(f"WARNING: {ANCHORS_FILE} 没有可用的 anchor(全是注释或 PLACEHOLDER),用空列表跑 — 仅验证 adapter 稳定性", file=sys.stderr)
    if args.smoke:
