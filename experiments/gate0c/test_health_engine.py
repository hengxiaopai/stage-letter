...
class HealthEngineGate0CTests(unittest.TestCase):
    ...
    def test_09_duplicate_sample_is_idempotent(self) -> None:
        engine = HealthEngine()
        sample = failure(1)
        first = engine.process(sample)
        duplicate = engine.process(sample)
    ...
    def test_14_failure_classes_do_not_change_live_state_semantics_here(self) -> None:
        # Gate 0C health has no LIVE/OFFLINE state at all. Different failure
        # classes only affect diagnostics; all follow the same health policy.
