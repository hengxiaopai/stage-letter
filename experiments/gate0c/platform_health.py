...
class HealthTracker:
    ...
    def process(self, sample: ProbeSample) -> HealthProcessResult:
        ...
        # Duplicate classification has precedence over stale classification.
        ...
        if sample.decisive:
            ...
            if sample.latency_ms >= self.config.slow_latency_ms:
                self.slow_samples += 1
                self.consecutive_clean_successes = 0
                # A slow decisive response proves reachability but not healthy
                # service quality. Even UNAVAILABLE recovers only to DEGRADED.
            ...
            # Any UNKNOWN fact is a health failure for the source path, even if
            # no lower-level error code is available. It remains UNKNOWN for
            # Gate 0B; health never coerces it into OFFLINE.
            ...
            if self.last_failure_kind in self.HARD_FAILURES:
                ...
                self.state = HealthState.DEGRADED
            # Before the degradation threshold, retain the existing health
            # state. STARTING therefore remains STARTING and HEALTHY tolerates
            # one transient failure by default.
...
def aggregate_health(states: Iterable[HealthState]) -> HealthState:
    ...
    if all(value is HealthState.STARTING for value in values):
        return HealthState.STARTING
    # Any mixed population means the platform path is only partially healthy.
