...
from platform_adapters.huya.adapter import HuyaAdapter  # noqa: E402
from platform_adapters.common import LiveStatus, ALL_STATES  # noqa: E402

ANCHORS_FILE = ROOT / "experiments" / "test_anchors" / "huya.txt"
...
def setup_logging(log_path: Path) -> logging.Logger:
    ...
    logger = logging.getLogger("huya_24h")
...
def run(duration_seconds: int, interval_seconds: int, anchors: list, soak_type: str = "correctness") -> dict:
    ...
    log_path = DATA_DIR / f"huya_24h-{ts}.log"
    jsonl_path = DATA_DIR / f"huya_24h-{ts}.jsonl"

    logger = setup_logging(log_path)
    adapter = HuyaAdapter(min_interval=max(2, interval_seconds / 2))

    logger.info("=== Huya 24h 稳定性实验启动 ===")
    ...
    summary_path = DATA_DIR / f"huya_24h-{ts}.summary.json"
...
def main():
    p = argparse.ArgumentParser(description="Huya 24h 稳定性实验")
