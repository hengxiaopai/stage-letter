...
if [[ "$(basename "$ROOT")" != "stage-letter" ]]; then
  echo "ERROR: run from the existing stage-letter project root." >&2
  exit 1
fi
...
if [[ ! -d .git ]]; then
  ...
  echo "INFO existing .git detected"
fi
...
if ! git config user.email >/dev/null 2>&1; then
  git config user.email "stage-letter-local@example.invalid"
fi
...
if git remote get-url origin >/dev/null 2>&1; then
  ...
  if [[ "$CURRENT_ORIGIN" != "$REPO_URL" && "$CURRENT_ORIGIN" != "git@github.com:hengxiaopai/stage-letter.git" ]]; then
    echo "ERROR: origin points elsewhere: $CURRENT_ORIGIN" >&2
...
echo
echo "NOTE: local mini-program is not pushed automatically."
