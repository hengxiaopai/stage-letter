#!/usr/bin/env python3
"""Provider-backed probe for Gate 1.3-4A Bilibili formal adapter."""
...
from stage_letter.infrastructure.platforms.bilibili_http import BilibiliHttpGateway
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Exercise the formal Bilibili provider chain.")
    parser.add_argument(
        "identity",
        help="Bilibili uid, space URL, or live-room URL",
    )
    parser.add_argument(
        "--expect",
...
async def _run(identity: str, expected: str) -> tuple[int, dict[str, object]]:
    ...
    gateway = BilibiliHttpGateway()
    adapter = BilibiliFormalAdapter(gateway)
    try:
