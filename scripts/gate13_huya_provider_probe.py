#!/usr/bin/env python3
"""Provider-backed probe for Gate 1.3-4B Huya formal adapter."""
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Exercise the formal Huya provider chain.")
    parser.add_argument(
        "identity",
        help="Huya room id, www.huya.com room URL, or m.huya.com room URL",
    )
    parser.add_argument(
        "--expect",
...
async def _run(identity: str, expected: str) -> tuple[int, dict[str, object]]:
    ...
    gateway = HuyaHttpGateway()
    adapter = HuyaFormalAdapter(gateway)
    try:
